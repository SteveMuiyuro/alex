...
locals {
  frontend_bucket_origin = "https://${var.frontend_bucket_name}.storage.googleapis.com"
  frontend_origin        = var.create_frontend_load_balancer && trimspace(var.frontend_domain) != "" ? "https://${trimspace(var.frontend_domain)}" : local.frontend_bucket_origin
  api_cors_origins = join(",", compact([
    "http://localhost:3000",
    "http://127.0.0.1:3000",
    local.frontend_bucket_origin,
    var.create_frontend_load_balancer && trimspace(var.frontend_domain) != "" ? local.frontend_origin : "",
  ]))
}
